...
extern double disSquareTwoPoint(double x1, double y1, double x2, double y2);

extern double myNchooseM(int n, int m);

/* If point z is above the line connected by point x and y */
template<class T>
inline bool pointAboveLine(pair<T, T> x, pair<T, T> y, pair<T, T> z)
{
...
template <class T>
inline bool pointInsideAngle(pair<T,T> a, pair<T, T> x, pair<T, T> y, pair<T, T> z) {
